    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                if (nis) {
                    response = await apis.getSiswaByNis(nis);
                }
                    ...
                    <div className="relative">
                        ...
                        <table className="min-w-full divide-y divide-gray-200 mt-4">
                            ...
                            <tbody className="bg-white divide-y divide-gray-200">
                                {siswa.map((swa) => (
                                    <tr key={swa.nis} className="bg-white dark:bg-gray-600 hover:bg-gray-50
                                        dark:hover:bg-gray-500 drak:text-white">
                                        <td className="px-6 py-4 font-sm whitespace-nowrap">{swa.nis}</td>
                                        <td className="px-6 py-4 font-sm whitespace-nowrap">{swa.nama}</td>
                                        <td className="px-6 py-4 font-sm whitespace-nowrap">{swa.angkatan}</td>
                                        <td className="px-6 py-4 font-sm whitespace-nowrap">{swa.jurusan}</td>
                                        ...
                                        <div className="flex flex-row justify-start">
                                            <button className="text-slate-800 underline cursor-pointer hover:text-blue-800"
                                                    onClick={() => { openModal(swa) }}>Edit</button>
                                            <button className="text-white text-md bg-red-700 py-1 px-2 cursor-pointer hover:bg-red-900 ml-4"
                                                    onClick={() => { handleDelete(swa.nis)}}>Hapus</button>
                                            </div>
                                        </td>
                                    </tr>
                                ))}
                            </tbody>
                            <button className="" onClick={()=>navigate("/createsiswa")}>TAMBAH DATA</button>
                        </table>
